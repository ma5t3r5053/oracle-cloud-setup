...
instance_image_version=$(oci compute pic version list --listing-id "$instance_image_ocid_listing" --raw-output --query "sort_by(data,&\"time-published\")[*].\"listing-resource-version\" | [0] ")
echo -e "\e[1;32m instance_image_version : $instance_image_version \e[0m"

instance_image_available=$(oci compute pic version get --listing-id "$instance_image_ocid_listing" --resource-version "$instance_image_version" --query 'data."compatible-shapes"|contains(@,`'$instance_shape'`)' --raw-output)
echo -e "\e[1;32m instance_image_available : $instance_image_available \e[0m"
